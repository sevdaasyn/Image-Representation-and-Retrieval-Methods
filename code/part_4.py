...
def find_five_most_similar(img, selected_q):
    ...
    for k in range(5):
        dist_in = 0.0
        for m in range(len(preprocessed_query)):
            dist_in = dist_in + distance.euclidean(preprocessed_query[m], preprocessed_trains[k])
        dist.append(dist_in)
...
for i in range(len(cv_query_images)):
    ...
    height, width, channels = cv_query_images[i].shape
